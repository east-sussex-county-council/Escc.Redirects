...
$projectName = "Escc.Redirects.Admin.MVC" 
...
robocopy $sourceFolder "$destinationFolder/$projectName" /MIR /IF *.dll *.css csc.* csi.* *.js *.cshtml *.asax *.gif *.scss *.png *.swf *.eot *.svg *.ttf *.wof /XD aspnet_client obj Properties App_Start
...
TransformConfig "$sourceFolder\Views\web.config" "$destinationFolder\$projectName\Views\web.config" "$transformsFolder\$projectName\Views\web.release.config"
...
CreateVirtualDirectory $websiteName "redirectsmvc" "$destinationFolder\$projectName" true "$projectName-$websiteName"
DisableAnonymousAuthentication $websiteName "redirectsmvc"
EnableWindowsAuthentication $websiteName "redirectsmvc"
